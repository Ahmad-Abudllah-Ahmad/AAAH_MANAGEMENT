...
async function getLibreDwg() {
  if (!libredwgPromise) {
    libredwgPromise = (async () => {
      try {
        const pkg = "@mlightcad/libredwg-web";
        const mod = await import(/* @vite-ignore */ pkg);
        const LibreDwg = mod.LibreDwg || mod.default?.LibreDwg || mod.default;
        const root = (import.meta.env.BASE_URL || "/").replace(/\/?$/, "/");
        return LibreDwg?.create ? LibreDwg.create(`${root}libredwg/`) : null;
      } catch (e) {
        console.warn("LibreDwg not available:", e);
        return null;
      }
    })().catch((e) => {
...
async function dwgToPdf(file) {
  const pkg = "@mlightcad/libredwg-web";
  const mod = await import(/* @vite-ignore */ pkg);
  const Dwg_File_Type = mod.Dwg_File_Type || mod.default?.Dwg_File_Type || { DWG: 0 };
  const libredwg = await getLibreDwg();
  if (!libredwg) throw new Error("LibreDWG engine is not available");
  const bytes = new Uint8Array(await file.arrayBuffer());
