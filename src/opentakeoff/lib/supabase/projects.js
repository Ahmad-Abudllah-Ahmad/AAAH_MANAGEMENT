...
export function navigateToSupabaseProject(projectId, navigate) {
  ...
  const currentPath = typeof window !== "undefined" && window.location.pathname.startsWith("/drawing-scanner")
    ? window.location.pathname
    : "/drawing-scanner/detect";
  const path = `${currentPath}?db=${encodeURIComponent(projectId)}`;
...
export function openSupabaseProject(projectId, navigate) {
  navigateToSupabaseProject(projectId, navigate);
}

/** Return to the home screen (recent projects list). */
export function goSupabaseHome(navigate) {
  const currentPath = typeof window !== "undefined" && window.location.pathname.startsWith("/drawing-scanner")
    ? window.location.pathname
    : "/drawing-scanner/detect";
  try {
    localStorage.removeItem("adicc_supabase_project_id");
  } catch { /* private mode */ }
  if (typeof navigate === "function") {
