...
export function createSupabaseRecents(storage = browserStorage()) {
  return {
    ...
    list() {
      ...
      return parsed
        .filter((e) => e && typeof e.id === "string" && typeof e.name === "string" && !e.name.toLowerCase().includes("arch. drawings part ii"))
...
export function mergeProjectLists(fromDb, recents, needle = "") {
  ...
  const isExcluded = (p) => {
    ...
    return n.includes("arch. drawings part ii") || n.includes("sheet previews unavailable");
  };
  const matches = (p) => !isExcluded(p) && (!q || (p.name || "").toLowerCase().includes(q));

  const ordered = [];
  const seen = new Set();

  for (const r of recents) {
    const row = fromDb.length > 0 ? byId.get(r.id) : { id: r.id, name: r.name, sheetCount: 0, shapeCount: 0, lastOpenedAt: r.openedAt ? new Date(r.openedAt).toISOString() : null, updatedAt: null };
