...
import { 
  ...
  X, Sparkles, Check, ArrowRight, ShieldCheck, Eye, Layers, ZoomIn
} from 'lucide-react';
...
export const InvoiceUploadModal = ({ isOpen, onClose, onUploaded }) => {
  ...
  const handleFileChange = (file) => {
    ...
    startOcrExtraction(file, url);
  };

  const startOcrExtraction = (file, fileUrl, preset = null) => {
    ...
    // Determine vendor name from filename or preset
    let guessedVendor = 'Al Habtoor Contracting & Precast LLC';
    let guessedId = `INV-${Math.floor(25000 + Math.random() * 9000)}`;
    let guessedAmount = 184320;
    ...
    if (preset) {
      ...
      if (name.includes('steel')) {
        guessedVendor = 'Emirates Steel Industries PJSC';
        guessedAmount = 156555;
      } else if (name.includes('concrete') || name.includes('ready')) {
        guessedVendor = 'Gulf Ready Mix Concrete LLC';
        guessedAmount = 62370;
      } else if (name.includes('mep') || name.includes('duct')) {
        guessedVendor = 'Al Quoz MEP Engineering LLC';
        guessedAmount = 89400;
      } else if (name.includes('doka') || name.includes('formwork')) {
        guessedVendor = 'Dutco Formwork Solutions';
        guessedAmount = 118400;
      }
      // Check if filename contains digits like 24817
      const numMatch = file.name.match(/\d{4,6}/);
      if (numMatch) {
        guessedId = `INV-${numMatch[0]}`;
      }
    }
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const invoicePayload = preset || {
            ...
            project: 'Al Barsha Tower — Plot 4',
            total: guessedAmount,
            subtotal: Math.round(guessedAmount / 1.05),
            vat: guessedAmount - Math.round(guessedAmount / 1.05),
            amount: `${Number(guessedAmount).toLocaleString('en-US', { minimumFractionDigits: 2 })} AED`,
            confidence: 98,
            status: 'Pending Review',
            items: [
              { id: 1, desc: 'Primary Structural Concrete Deliveries', unit: 'm³', qty: 240, rate: 375, amount: 90000, poQty: 240, grnQty: 240, status: 'Matched' },
              { id: 2, desc: 'High-Tensile Rebar 20mm Grade 60', unit: 'MT', qty: 18, rate: 3150, amount: 56700, poQty: 18, grnQty: 18, status: 'Matched' },
              { id: 3, desc: 'Modular Alu-Decking Shoring Rig', unit: 'm²', qty: 350, rate: 82, amount: 28700, poQty: 350, grnQty: 350, status: 'Matched' },
            ],
            hasVariance: false,
            fileUrl: fileUrl,
            fileName: fileName,
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(8,30,60,0.65)', backdropFilter: 'blur(6px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <motion.div 
        ...
        style={{ 
          width: '100%', 
          maxWidth: parsedPreview ? 820 : 560, 
          background: 'white', 
          borderRadius: 16, 
        ...
        <div style={{ padding: '18px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#081E3C', color: 'white' }}>
          <div>
            ...
            <p style={{ margin: '2px 0 0 0', fontSize: 12, color: '#94A3B8' }}>
              Upload any PDF or image tax invoice for instant 3-way matching and bounding box extraction
            </p>
          </div>
        ...
        <div style={{ padding: 24, overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <>
              ...
              >
                ...
                <div style={{ fontSize: 15, fontWeight: 800, color: '#081E3C', marginBottom: 4 }}>
                  {isProcessing ? 'Running Deep Neural OCR Scan...' : 'Click to Upload PDF / Image or Drag & Drop'}
                </div>
                ...
                {isProcessing && (
                  <div style={{ marginTop: 20, maxWidth: 360, margin: '20px auto 0 auto' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, fontWeight: 700, color: '#004753', marginBottom: 6 }}>
                      <span>
                        {ocrStage === 1 && '1/3 Preprocessing & Bounding Box Detection...'}
                        {ocrStage === 2 && '2/3 Tokenizing Numerical Line Items...'}
          ...
          {parsedPreview && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11.5, color: '#047857' }}>
                      Identified {parsedPreview.items.length} line items mapped to {parsedPreview.poMatch} ({parsedPreview.project})
                    </div>
                  </div>
                </div>
                <button 
              ...
              {/* Exact Layout Visual Sheet Preview with Optical Bounding Boxes */}
              <div style={{ background: '#F8FAFC', borderRadius: 12, border: '1px solid #CBD5E1', padding: 20, position: 'relative', overflow: 'hidden' }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: '#004753', textTransform: 'uppercase', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
                  <ShieldCheck size={15} color="#00A9C5" /> Optical Bounding Box Exact Field Positioning
                </div>

                {/* Printable Invoice Mock Layout */}
                <div style={{ background: 'white', padding: 20, borderRadius: 8, border: '1px solid #E2E8F0', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                  
                  {/* Top Header */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                    {/* Supplier Bounding Box */}
                    <div style={{ border: '1.5px solid #00A9C5', background: '#F0F8FA', padding: '10px 14px', borderRadius: 6, position: 'relative', maxWidth: 340 }}>
                      <div style={{ position: 'absolute', top: -8, right: 8, background: '#004753', color: 'white', fontSize: 9.5, padding: '1px 5px', borderRadius: 3, fontWeight: 800 }}>
                        OCR: VENDOR [99%]
                      </div>
                      <div style={{ fontSize: 10, color: '#00556A', fontWeight: 800, textTransform: 'uppercase' }}>SUPPLIER / المورد</div>
                      <div style={{ fontSize: 13.5, fontWeight: 900, color: '#081E3C' }}>{parsedPreview.vendor}</div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: '1.5px solid #00A86B', background: '#F0FDF4', padding: '3px 8px', borderRadius: 4, marginTop: 4 }}>
                        <span style={{ fontSize: 11.5, fontWeight: 800, color: '#081E3C' }}>{parsedPreview.id}</span>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5, border: '1px solid #CBD5E1', marginBottom: 12 }}>
                    <thead style={{ background: '#081E3C', color: 'white' }}>
                      <tr>
                        ...
                        <th style={{ padding: '6px 8px', textAlign: 'center', width: 50 }}>Unit</th>
                        <th style={{ padding: '6px 8px', textAlign: 'right', width: 60 }}>Qty</th>
                        <th style={{ padding: '6px 8px', textAlign: 'right', width: 90 }}>Rate (AED)</th>
                    ...
                    <tbody>
                      {parsedPreview.items.map((item, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid #E2E8F0', background: idx % 2 === 0 ? 'white' : '#FAFAFA' }}>
                          ...
                          <td style={{ padding: '8px', fontWeight: 700, color: '#081E3C' }}>{item.desc}</td>
                          <td style={{ padding: '8px', textAlign: 'center', color: '#64748B' }}>{item.unit}</td>
                          <td style={{ padding: '8px', textAlign: 'right', fontWeight: 700 }}>{item.qty}</td>
                          <td style={{ padding: '8px', textAlign: 'right', color: '#334155' }}>{Number(item.rate).toLocaleString()}</td>
                          <td style={{ padding: '8px', textAlign: 'right', fontWeight: 800, color: '#081E3C' }}>{Number(item.amount).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <div style={{ width: 220, border: '1.5px solid #004753', background: '#F0F8FA', padding: '8px 12px', borderRadius: 6 }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 900, color: '#004753', marginTop: 4, paddingTop: 4, borderTop: '1px solid rgba(0,71,83,0.2)' }}>
                        <span>Total:</span>
